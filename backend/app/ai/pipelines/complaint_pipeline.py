...
from app.ai.claude.parser import parse_json_object
from app.ai.claude.retry_handler import with_retries
...
class ComplaintAIPipeline:
    ...
    async def process(self, complaint: ComplaintProcessRequest) -> tuple[AIEnrichment, LocalSignals]:
        ...
        async def call_claude() -> str:
            return await self.claude.analyze_complaint(
        ...
        raw_response = await with_retries(call_claude, self.settings.ai_max_retries)
