...
export default function RegulatoryRagPage() {
  ...
  const [searchRegulator, setSearchRegulator] = useState<ComplianceRegulator | "">("RBI");
  const [domain, setDomain] = useState("");
  const [chunkStatus, setChunkStatus] = useState<(typeof CHUNK_STATUSES)[number] | "">("draft");
  ...
  const canUpload = Boolean(file && documentTitle.trim() && version.trim() && !uploading);
...
function UploadPanel(props: {
  ...
  return <div className="card"><div className="card-header"><div style={{ display: "flex", alignItems: "center", gap: 8 }}><Upload size={16} /><strong>Guideline Upload</strong></div></div><div className="card-body" style={{ display: "grid", gap: 12 }}>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "120px 1fr", gap: 12 }}><Field label="Regulator"><select className="form-select" value={props.regulator} onChange={(e) => props.setRegulator(e.target.value as ComplianceRegulator)}>{REGULATORS.map((item) => <option key={item} value={item}>{item}</option>)}</select></Field><Field label="Version"><input className="form-input" value={props.version} onChange={(e) => props.setVersion(e.target.value)} placeholder="2026.1" /></Field></div>
    <Field label="Document Title"><input className="form-input" value={props.documentTitle} onChange={(e) => props.setDocumentTitle(e.target.value)} placeholder="RBI Master Direction" /></Field>
