...
Switched the Phase 1 AI provider to OpenAI:

- Replaced the AI runtime with `backend/app/ai/openai/`.
- Removed the old provider package and old provider dependencies.
- Added `OPENAI_API_KEY`, `OPENAI_MODEL`, and optional `OPENAI_BASE_URL` settings.
- Backend startup now checks database readiness and OpenAI access before serving requests.
- Updated `.env.template`, API docs, setup docs, DevOps notes, and handoff docs for OpenAI.
...
- Add a real `OPENAI_API_KEY` to local `.env`.
...
- Real OpenAI key and model access from Harsh/team.
