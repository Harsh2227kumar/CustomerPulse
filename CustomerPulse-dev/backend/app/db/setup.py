...
class DatabaseSetupStatus:
    ...
    agent_feedback_table_exists: bool
    missing_complaint_columns: tuple[str, ...]
    missing_complaint_indexes: tuple[str, ...]
    missing_feedback_columns: tuple[str, ...]
    missing_feedback_indexes: tuple[str, ...]
    ...
    def schema_ready(self) -> bool:
        return (
            ...
            and self.agent_feedback_table_exists
            and not self.missing_complaint_columns
            and not self.missing_complaint_indexes
            and not self.missing_feedback_columns
            and not self.missing_feedback_indexes
        )
...
EXPECTED_FEEDBACK_COLUMNS: dict[str, str] = {
    "id": "VARCHAR(64)",
...
async def inspect_database(settings: Settings) -> DatabaseSetupStatus:
    if not await database_exists(settings):
        return DatabaseSetupStatus(
            ...
            agent_feedback_table_exists=False,
            missing_complaint_columns=tuple(EXPECTED_COMPLAINT_COLUMNS),
            missing_complaint_indexes=tuple(index.name or "" for index in Complaint.__table__.indexes),
            missing_feedback_columns=tuple(EXPECTED_FEEDBACK_COLUMNS),
            missing_feedback_indexes=tuple(index.name or "" for index in AgentFeedback.__table__.indexes),
        )
    ...
    try:
        async with engine.connect() as conn:
            ...
            complaint_table_result = await conn.execute(text("SELECT to_regclass('public.complaints')"))
            feedback_table_result = await conn.execute(text("SELECT to_regclass('public.agent_feedback')"))
            complaints_table_exists = complaint_table_result.scalar_one_or_none() is not None
            agent_feedback_table_exists = feedback_table_result.scalar_one_or_none() is not None
            existing_complaint_index_names: set[str] = set()
            existing_complaint_column_names: set[str] = set()
            existing_feedback_index_names: set[str] = set()
            existing_feedback_column_names: set[str] = set()
            if complaints_table_exists:
                existing_indexes = await conn.run_sync(
                    lambda sync_conn: inspect(sync_conn).get_indexes("complaints")
                )
                existing_complaint_index_names = {index["name"] for index in existing_indexes}
                existing_columns = await conn.run_sync(
                    lambda sync_conn: inspect(sync_conn).get_columns("complaints")
                )
                existing_complaint_column_names = {column["name"] for column in existing_columns}
            if agent_feedback_table_exists:
                existing_indexes = await conn.run_sync(
                    lambda sync_conn: inspect(sync_conn).get_indexes("agent_feedback")
                )
                existing_feedback_index_names = {index["name"] for index in existing_indexes}
                existing_columns = await conn.run_sync(
                    lambda sync_conn: inspect(sync_conn).get_columns("agent_feedback")
                )
                existing_feedback_column_names = {column["name"] for column in existing_columns}
            expected_complaint_index_names = {
                index.name for index in Complaint.__table__.indexes if index.name
            }
            expected_feedback_index_names = {
                index.name for index in AgentFeedback.__table__.indexes if index.name
            }
            missing_complaint_columns = tuple(
                column_name
                for column_name in EXPECTED_COMPLAINT_COLUMNS
                if column_name not in existing_complaint_column_names
            )
            missing_feedback_columns = tuple(
                column_name
                for column_name in EXPECTED_FEEDBACK_COLUMNS
                if column_name not in existing_feedback_column_names
            )
            missing_complaint_indexes = tuple(
                sorted(expected_complaint_index_names - existing_complaint_index_names)
            )
            missing_feedback_indexes = tuple(
                sorted(expected_feedback_index_names - existing_feedback_index_names)
            )
            return DatabaseSetupStatus(
                database_exists=True,
                vector_extension_exists=vector_result.scalar_one_or_none() == 1,
                complaints_table_exists=complaints_table_exists,
                agent_feedback_table_exists=agent_feedback_table_exists,
                missing_complaint_columns=missing_complaint_columns,
                missing_complaint_indexes=missing_complaint_indexes,
                missing_feedback_columns=missing_feedback_columns,
                missing_feedback_indexes=missing_feedback_indexes,
            )
...
async def create_schema(settings: Settings) -> None:
    ...
    try:
        ...
        async with engine.begin() as conn:
            ...
            for index in AgentFeedback.__table__.indexes:
                await conn.run_sync(index.create, checkfirst=True)
    finally:
        await engine.dispose()
...
async def ensure_database_ready(settings: Settings, *, prompt: bool = True) -> DatabaseSetupStatus:
    ...
    if not status.schema_ready:
        ...
        if not status.agent_feedback_table_exists:
            missing.append("agent_feedback table")
        if status.missing_complaint_columns:
            missing.append(
                f"complaint columns: {', '.join(status.missing_complaint_columns)}"
            )
        if status.missing_complaint_indexes:
            missing.append(
                f"complaint indexes: {', '.join(status.missing_complaint_indexes)}"
            )
        if status.missing_feedback_columns:
            missing.append(f"feedback columns: {', '.join(status.missing_feedback_columns)}")
        if status.missing_feedback_indexes:
            missing.append(f"feedback indexes: {', '.join(status.missing_feedback_indexes)}")
        if not prompt or not await _prompt_yes_no(
